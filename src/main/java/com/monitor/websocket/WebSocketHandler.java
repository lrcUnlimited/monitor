...
public class WebSocketHandler {
	...
	public void onMessage(String msg) throws IOException, InterruptedException {
		...
		String sendMsg = new StringBuffer().append(msg).append(msg).append(msg)
				.toString();
		 ...
		 * Message sendMessage = new Message();// 发送消息 Date nowDate = new
		 * Date();// 当前日期
		 * 
		 * // 检查设备的状态 Device device = deviceRepository.findOne(deviceId); //
		 * 设备状态已经为off，直接关机 if (device.getManageDeviceStatus() == 0) {
		 * sendMessage.setType(0); } else { if
		 * (nowDate.after(device.getValidTime())) { // 设备已经过期,更新数据库中的设备管理状态为off
		 * deviceRepository.updateManageDeviceStatus(0, deviceId);
		 * sendMessage.setType(0);// 发送关机指令
		 * 
		 * } else { if (SessionKeyUtil.isValidSessionKey(
		 * reciveMessage.getSessionKey(), device.getSessionKey())) {
		 * sendMessage.setSessionKey(reciveMessage.getSessionKey()); } else { //
		 * 如果sessionKey不合法,重新生成 String newSessionKey =
		 * SessionKeyUtil.generateSessionKey(); // 数据库中更新sessionkey
		 * deviceRepository.updateDeviceSessionKey(newSessionKey, deviceId); //
		 * 设置发送消息的sessionKey sendMessage.setSessionKey(newSessionKey); }
		 * sendMessage.setType(1);// 设置状态为允许开机 } }
		 * 
		 * sessionDevice.getBasicRemote().sendText(JSON.toJSONString(sendMessage)
		 * );// 发送消息
		 */
